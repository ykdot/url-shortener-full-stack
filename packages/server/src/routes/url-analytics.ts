...
router.get(
  '/url-table/:filter/:order/:page/:wordFilter',
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const getMainTableQuery = `SELECT date, long_url, urls.short_code, url_analytics.clicks FROM urls LEFT JOIN url_analytics ON urls.short_code = url_analytics.short_code WHERE long_url ILIKE '%${wordFilter}%' ORDER BY ${filter} ${order} LIMIT ${pageLimit + 1} OFFSET ${offset};`;
